...
struct HelloWorldView: View {
    ...
    @State private var tabIDsWithTitleAndHost: [TabInfoWithID] = []
    @State private var notificationObserver: NSObjectProtocol?
    @State private var keyMonitors: [Any] = []
    @State private var searchQuery: String = ""

    var filteredTabs: [TabInfoWithID] {
            if searchQuery.isEmpty {
                return tabIDsWithTitleAndHost.reversed()
            } else {
                return tabIDsWithTitleAndHost.reversed().filter { tab in
    ...
    var body: some View {
        VStack {
            ...
            ScrollViewReader { proxy in
                ScrollView(.vertical) {
                    VStack(spacing: 0) {
                        ForEach(filteredTabs.indices, id: \.self) { tabIdx in
                            let tab = filteredTabs[tabIdx]
                            ...
                            VStack(alignment: .leading, spacing: 15) {
                                ...
                                .lineLimit(1)
                                .padding(.top, 10).padding(.bottom, tabIdx != filteredTabs.indices.last ? 10 : 20)
                            ...
                            if tabIdx != filteredTabs.indices.last && tabIdx != filteredTabs.indices.first {
        ...
        .onAppear {
            tabIDsWithTitleAndHost = OrderedSet(Store.tabIDsWithTitleAndHost).elements
            NSApp.hide(nil)
